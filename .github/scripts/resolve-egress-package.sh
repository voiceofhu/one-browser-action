...
egress_repository="${EGRESS_REPOSITORY_INPUT:-voiceofhu/one-browser-egress}"
egress_ref="${EGRESS_REF_INPUT:-}"
version_tag="${VERSION_TAG_INPUT:-}"

if [ "$egress_repository" != "voiceofhu/one-browser-egress" ]; then
  echo "Egress packages only accept voiceofhu/one-browser-egress." >&2
  exit 1
fi
