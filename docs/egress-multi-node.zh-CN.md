...
- 创建仅允许 SSH 公钥登录的 `gh-deploy`，加入 `docker` 与
  `one-browser-deploy`；账号使用不可匹配的密码哨兵，但不会被 sshd 判定为 locked；
...
如果服务器已经安装 Docker，只需要创建或修复统一的部署账号，可以单独运行：

```bash
sudo bash .github/actions/egress/setup-gh-deploy-user.sh
```

该脚本固定安装 `gh-deploy@one-browser` 公钥，将 `gh-deploy` 加入 `docker` 和
...
监听和证书路径均一致时才会保留。重跑也会修复旧版本留下的 locked `gh-deploy`
账号并重新核对传入公钥。发现冲突的 Docker 软件包、不匹配的 `.env` 或被占用的
`80` 时，脚本会停止，不会擅自覆盖或删除。
